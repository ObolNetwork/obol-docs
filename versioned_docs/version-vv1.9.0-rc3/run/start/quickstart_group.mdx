        ...
        <p>
          ...
          <ol>
            ...
            <li>
              ...
              <pre>
                docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.9.0-rc3 create dkg {'\n'}
                --name="Quickstart" {'\n'}
                --num-validators=1 {'\n'}
...
3. Assuming the DKG is successful, a number of artefacts will be created in the `.charon` folder of the node. These include:
  ...
  <TabItem value="CLI" label="CDVN">
    Once the creator gives you the <code>cluster-definition.json</code> file and you place it in a <code>.charon</code> subdirectory, run: <pre>docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.9.0-rc3 dkg --publish</pre> and the DKG process should begin.
  </TabItem>
