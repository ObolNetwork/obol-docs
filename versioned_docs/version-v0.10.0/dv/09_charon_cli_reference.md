...
Available Commands:
  ...
  completion  Generate the autocompletion script for the specified markdown
...
This command starts a [discV5](https://github.com/ethereum/devp2p/blob/master/discv5/discv5.md) bootnode server that charon nodes use to discover each other in the cluster. Bootnodes can also be used as [libp2p circuit relay](https://docs.libp2p.io/concepts/circuit-relay/) which allows peers in a cluster to connect to each other through the bootnode. This is helpful if peers are behind NAT gateways.
