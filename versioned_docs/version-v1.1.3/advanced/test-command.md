...
```shell
docker run -v /Users/obol/charon/.charon:/opt/charon/.charon obolnetwork/charon:v1.1.3 alpha test peers \
...
```shell
docker run obolnetwork/charon:v1.1.3 alpha test beacon \
  --endpoints="https://ethereum-holesky-beacon-api.publicnode.com,https://ethereum-sepolia-beacon-api.publicnode.com"
```
...
```shell
docker run obolnetwork/charon:v1.1.3 alpha test validator
```
...
```shell
docker run obolnetwork/charon:v1.1.3 alpha test performance
```

For proper examination of the disk performance, it is advised to run the tests in a Docker volume.

```shell
docker run -v ${HOME}/charon/test:/opt/charon/test obolnetwork/charon:v1.1.3 alpha test performance \
