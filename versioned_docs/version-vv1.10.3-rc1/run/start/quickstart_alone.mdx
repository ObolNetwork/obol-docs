...
<Tabs groupId="Launchpad-other">
  ...
  docker run -u $(id -u):$(id -g) --rm -v "$(pwd)/:/opt/charon" obolnetwork/charon:v1.10.3-rc1 create cluster --definition-file=...
  ```
...
2. Run the cluster creation command, setting required flag values.
  ...
  ```shell
    docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.10.3-rc1 create cluster \
      --nodes=6 \
      --network=holesky \
      --num-validators=1 \
