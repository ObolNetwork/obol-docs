...
This quick start guide for the Sedge integration explains how to launch a Distributed Validator (DV) node using Sedge, a user-friendly, one-click setup tool for running an Ethereum validator. With Sedge, you can easily deploy and manage a validator without needing prior technical knowledge.

The guide focuses on deploying and running a validator on the Holesky testnet. However, the same steps can be applied to the mainnet by simply updating the relevant environment variables.

This guide is tailored for Mac operating system users. However, for installation on other operating systems, such as Linux or Windows, please refer to the <a href="https://docs.sedge.nethermind.io/docs/quickstart/install-guide" target="_blank">official Sedge installation guide</a>

## Minimum Holesky Requirements 
- 4 CPU cores
- 8 GB RAM
...
For up-to-date hardware considerations, check out the <a href="https://ethereum.org/en/developers/docs/nodes-and-clients/run-a-node/#environment-and-hardware" target="_blank">ethereum.org guides</a> which explores various setups and trade-offs, such as running the node locally or in the cloud. 

## Build from source using 'make'  
In order to build from source, make sure you have the following software dependencies installed: 

- Git - Git is a widely used version control system. You can install it using Hombrew.
- GoLang -  install the Go programming language following the <a href="https://go.dev/doc/install" target="_blank">official instructions</a>.  You need at least version `1.20`.

In your mac terminal, run the following commands to install sedge. 
<p>
          <ol>
            <li>
              Install make
            ...
            <li>
              Clone git repo
              ```shell
              git clone https://github.com/ObolNetwork/sedge 
              ```
            </li>
            <li>
            ...
            <li>
               In case you want the binary in your PATH please copy the binary to `/usr/local/bin:`
              ```shell
...
To install Sedge via Homebrew, you’ll first need to install the Homebrew package manager on your Mac. Use the official installation script provided on the <a href="https://brew.sh/" target="_blank">Homebrew homepage</a>.
<p>
          <ol>
            <li>
              Once Homebrew is installed, you’re ready to set up Sedge. Simply add the official Sedge repository and proceed with the installation:
              ```shell
...
<TabItem value="Sedge" label="Sedge">
## Sedge generate 
With Sedge now installed, it’s time to deploy a distributed validator. Using the `sedge generate` command and its subcommands, Sedge will create a Docker Compose file needed to run the validator node.
<p>
          <ol>
            <li>
              The following command generates the artifacts required to deploy a distributed validator on the Holesky network, using Teku as the validator client, Prysm as the consensus client, and Geth as the execution client. For additional supported client combinations, <a href="https://github.com/ObolNetwork/sedge?tab=readme-ov-file#supported-networks-and-clients" target="_blank">refer to the documentation here</a>.
              ```shell
            ...
            <li>
              ...
              sedge-data
              ```
              change dir and list the files inside sedge-data
               ```shell
              > cd sedge-data 
              > ls 
...
## Sedge Import-key
Ideally you would do your charon DKG process before deploying your DV node. Once you have successfully completed your DKG process, it would generate the DV keys which you will need to import into sedge using the `sedge import-key command.` 

Use the following command to import keys from the directory where the .charon dir is located. In the below example, node0 is the directory where you will find the .charon folder.
```shell
sedge import-key --from node0 holesky teku 
```
## Sedge Run
