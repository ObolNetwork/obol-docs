...
If you are running the tests towards another Docker container, you need to include its network as well. Check docker networks with `docker network ls` and specify the Docker network with `--network <name>`. This is the case with running Charon cluster using the [charon-distributed-validator-node repository](https://github.com/ObolNetwork/charon-distributed-validator-node/), as usually services like the beacon node and validator client are hosted locally. Read more about docker networking [here](https://docs.docker.com/engine/network/).
