...
3. Create the artifacts needed to run a DV cluster by running the following command:
   ...
   docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v0.19.1 create cluster --name="mycluster" --cluster-dir=".charon/cluster/" --withdrawal-addresses="${WITHDRAWAL_ADDR}" --fee-recipient-addresses="${FEE_RECIPIENT_ADDR}" --nodes 6 --network goerli --num-validators=1
   ```
